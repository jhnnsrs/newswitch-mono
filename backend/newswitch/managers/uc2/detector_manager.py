...
class VirtualDetectorManager:
    ...
    def _get_active_detectors(self) -> list[Detector]:
        ...
        return [d for d in self.camera_state.detectors if d.is_active]
    ...
    def list_available_detectors(self) -> list[Detector]:
        ...
        """
        return self.camera_state.detectors
    ...
    def acquire_live(self) -> None:
        ...
        while True:
            if self.camera_state.is_acquiring is False:
                koil.sleep(1)
            else:
                active_detectors = self._get_active_detectors()
                if not active_detectors:
                ...
                for detector in active_detectors:
                    frame = self.capture_image(detector.slot)
                    self.broadcaster.broadcast_sync(detector.slot, frame)

                # Use exposure time from first active detector for frame interval
                exposure_time = active_detectors[0].current_exposure_time
                koil.sleep(exposure_time / 1000.0)  # Simulate frame interval
