...
const toRevisionAtTime = (boundary: AppTimelineBoundary, targetMs: number) => {
  ...
  const progress = (clampedMs - boundary.startMs) / (boundary.endMs - boundary.startMs);

  return Math.round(
    boundary.startRevision + progress * (boundary.endRevision - boundary.startRevision),
  );
};
...
function TimelineFloater() {
  ...
  const [debouncedSelectedMs, setDebouncedSelectedMs] = useState<number | null>(null);
  ...
  const loadTimeline = useCallback(async () => {
    ...
    try {
      ...
      const timelineEndMs = Math.max(...nextBoundaries.map((boundary) => boundary.endMs));
      ...
      try {
        await Promise.all(
          boundaries.map(async (boundary) => {
            const nextRevision = toRevisionAtTime(boundary, debouncedSelectedMs);

            if (lastCheckoutRevisionRef.current[boundary.appKey] === nextRevision) {
              return;
            }
        ...
        const message =
          error instanceof Error ? error.message : "Failed to checkout timeline state";
        toast.error(message);
      } finally {
        if (!cancelled) {
              ...
              >
                <div className="flex min-w-0 flex-col gap-3 px-3 py-2">
                  <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-muted-foreground">
                    <div className="font-medium">{formatTimelineLabel(timelineBounds.startMs)}</div>
                    <div className="flex items-center gap-2 rounded-full border border-border/60 bg-background/70 px-3 py-1 text-foreground">
                      {isCheckingOut ? <Loader2 className="size-3.5 animate-spin" /> : null}
                  ...
                  <div className="flex flex-wrap gap-2 text-[11px] text-muted-foreground">
                    {boundaries.map((boundary) => (
                      ...
                      >
                        <span className="font-semibold text-foreground">{boundary.appKey}</span>
