...
function normalizeLockCollection(payload: LockCollectionResponse): Record<string, string | undefined> {
	return Object.fromEntries(
		Object.entries(payload.locks).map(([key, value]: [string, LockView]) => [key, value.task_id ?? undefined]),
	);
}
...
export function BundleProvider({ children }: BundleProviderProps) {
						...
						listener: (message) => {
							...
							switch (message.type) {
								...
								case TaskEventType.TASK_INIT:
									taskStore.upsertTasks(normalizeTaskCollection(appKey, message));
									return;
								...
								case TaskEventType.PAUSED:
									taskStore.updateTask(message.assignation, { status: 'paused' });
									return;
								case TaskEventType.RESUMED:
									taskStore.updateTask(message.assignation, { status: 'running' });
									return;
								case TaskEventType.CANCELLED:
									taskStore.updateTask(message.assignation, { status: 'cancelled' });
									return;
								case TaskEventType.INTERRUPTED:
									taskStore.updateTask(message.assignation, { status: 'interrupted' });
									return;
			...
			if (!connectionSubscriptionsRef.current.has(appKey)) {
				connectionSubscriptionsRef.current.set(
					...
					transport.subscribeToConnectionState(appKey, (state) => {
						...
						appStateStoreRegistry.getStoreApi(appKey).getState().setConnectionState(state);
						syncConnectionState();
					}),
				);
			}
	...
	const cleanupIfAppNotLive = useCallback(
		...
		[appStateStoreRegistry, getLiveCount, globalStateStoreRegistry, syncConnectionState],
	);
		...
		async <T extends Record<string, unknown>, TKey extends string>(
			...
			const request = transport
				...
				.then((response) => {
					...
					if (!parsed.success) {
						console.error(`[BundleProvider] Validation failed for ${definition.appKey}.${definition.key}`, {
							error: parsed.error,
				...
				.catch((error) => {
					const normalizedError = normalizeError(error, `${definition.appKey}.${definition.key}`);
	...
	const cancelTask = useCallback(
		async (appKey: AppKey, taskId: string) => {
			await updateTaskStatus(taskId, 'cancelled', () => transport.cancelTaskRequest(appKey, taskId), appKey);
		},
	...
	const pauseTask = useCallback(
		async (appKey: AppKey, taskId: string) => {
			await updateTaskStatus(taskId, 'paused', () => transport.pauseTaskRequest(appKey, taskId), appKey);
		},
		[transport, updateTaskStatus],
	);
	...
	const unpauseTask = useCallback(
		async (appKey: AppKey, taskId: string) => {
			await updateTaskStatus(taskId, 'running', () => transport.unpauseTaskRequest(appKey, taskId), appKey);
		},
		[transport, updateTaskStatus],
	);
	...
	const stepTask = useCallback(
		async (appKey: AppKey, taskId: string) => {
			await updateTaskStatus(taskId, 'running', () => transport.stepTaskRequest(appKey, taskId), appKey);
		},
		[transport, updateTaskStatus],
	);

	const subscribeToTask = useCallback(
		(taskId: string, appKey: AppKey, callback: (task: Task) => void): (() => void) => {
			const storeApi = taskStoreRegistry.getStoreApi(appKey);
	...
	const waitForTask: TaskContextValue['waitForTask'] = useCallback(
		<TArgs = unknown, TReturn = unknown>(appKey: AppKey, taskId: string): Promise<Task<TArgs, TReturn>> => {
			...
			if (cachedTask?.status === 'cancelled' || cachedTask?.status === 'interrupted') {
			...
			return new Promise<Task<TArgs, TReturn>>((resolve, reject) => {
				const unsubscribe = subscribeToTask(taskId, appKey, (task) => {
					...
					if (typedTask.status === 'cancelled' || typedTask.status === 'interrupted') {
						unsubscribe();
		...
		<AppStateContext.Provider value={appStateValue}>
			<StateContext.Provider value={stateValue}>
				<TaskContext.Provider value={taskValue}>
					<LockContext.Provider value={lockValue}>{children}</LockContext.Provider>
